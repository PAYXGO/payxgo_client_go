# sdk引入
        go get github.com/PAYXGO/payxgo_client_go@v0.0.1
# 说明
* 此版本为golang版本调用
* 请求调用频率为每分钟一次
...
# 传参说明
```go
func payxgo_client.New(urlPath string, secretKey string, accessKey string, action payxgo_client.action, c *payxgo_client.Config, cookieValue ...string) *payxgo_client.payxgoClient
```
* urlPath: 支付服务器地址，在用户注册端，  可在api token页面获取
...
// 获取支付链接和支付信息
	config := payxgo_client.New("https://www.saleoner.com/go/v1/transactions/securepay",
		...
		payxgo_client.Pay, &payxgo_client.Config{
			...
			OrderNum: "altsjdglkiuaiia5",   // 客户订单号(必须具有唯一性， 否则将请求失败)
